    ...
    describe('fromJSON()', () => {
        ...
        context('when the json is undefined', () => {
            it('should return the class without parameters', () => {
                ...
                assert.isUndefined(result.weekly_feeding_habits)
                assert.isUndefined(result.daily_water_glasses)
                assert.isUndefined(result.six_month_breast_feeding)
                assert.isUndefined(result.food_allergy_intolerance)
                assert.isUndefined(result.breakfast_daily_frequency)
                assert.propertyVal(result, 'type', DefaultEntityMock.FEEDING_HABITS_RECORD.type)
            })
        })
