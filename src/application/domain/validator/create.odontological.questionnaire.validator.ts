...
export class CreateOdontologicalQuestionnaireValidator {
    public static validate(item: OdontologicalQuestionnaire): void | ValidationException {
        ...
        else CreateSociodemographicRecordValidator.validate(item.sociodemographic_record)
        if (!item.family_cohesion_record) fields.push('family_cohesion_record')
        else CreateFamilyCohesionRecordValidator.validate(item.family_cohesion_record)
        if (!item.oral_health_record) fields.push('oral_health_record')
        else CreateOralHealthRecordValidator.validate(item.oral_health_record)
